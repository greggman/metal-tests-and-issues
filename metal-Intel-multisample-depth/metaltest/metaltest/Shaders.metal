...
fragment float4 depthTextureFShader(VertexOut interpolated [[stage_in]],
                                    metal::depth2d<float> texture [[texture(0)]],
                                    metal::sampler sampler [[sampler(0)]])
{
    float d = texture.sample(sampler, interpolated.uv);
...
fragment float4 zFShader(VertexOut interpolated [[stage_in]])
{
    float d = interpolated.pos.z;
    return float4(d, d, d, d);
}

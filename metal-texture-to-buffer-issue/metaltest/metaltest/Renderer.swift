...
class Renderer : NSObject, MTKViewDelegate {
  ...
  func draw(in view: MTKView) {
    ...
    for texInfo in textureTypes
    {
      ...
      for ndx in start..<formats.count
      {
        ...
        let width = 3
        let height = 4
        let depth = textureType == .type3D ? 5 : 1
        let dstBufferBytesPerRow = 8192
        let dstBufferBytesPerImage = max(8192 * height, width * height * depth * format.bytesPerPixel);
        ...
        let copySize = MTLSize(width: width, height: height, depth: depth)
        let destinationOffset = 0

        print("\nFormat: \(format.name), type: \(texInfo.name) texSize: \(width), \(height), \(depth), copySize: \(copySize.width), \(copySize.height), \(copySize.depth), destinationOffset: \(destinationOffset), destinationBytesPerRow: \(dstBufferBytesPerRow), destinationBytesPerImage: \(dstBufferBytesPerImage)")

        let texture = device.makeTexture(descriptor: textureDescriptor)!
        let region = MTLRegion(origin: MTLOrigin(x: 0, y: 0, z: 0), size: copySize)

        let numBytes = format.bytesPerPixel * width * height * depth;
        ...
        texture.replace(
            ...
            slice: 0,
            withBytes: data,
            bytesPerRow: format.bytesPerPixel * width,
            bytesPerImage: depth == 1 ? 0 : format.bytesPerPixel * width * height)
  ...
  func checkContents(buffer: MTLBuffer, bufferSize: Int, bytesPerPixel: Int, bufferBytesPerRow: Int, bufferBytesPerImage: Int, width: Int, height: Int, depth: Int, dataStartValue: UInt8) -> Bool
  {
    ...
    for i in 0..<bufferSize {
      let slice = i / bufferBytesPerImage
      let row = i % bufferBytesPerImage / bufferBytesPerRow
      let byteX = i % bufferBytesPerImage % bufferBytesPerRow
      let x = byteX / bytesPerPixel
      let value = data[i]
      let srcOffset = (slice * width * height + row * width) * bytesPerPixel + byteX;
      let inBounds = slice < depth && row < height && x < width

...
func runOnDevice(on device: MTLDevice) {
    ...
    fragment float4 fragment_main(VertexOut in [[stage_in]],
        ...
        float4 result = float4(texture.sample_compare(smp, float2(0.3), ref, level(0u), int2(0)));
        return result;
    }
    """
    ...
    let textureData: [Float32] = [0.3, 0.05, 0.85, 0.45]
    print(String(format: "Texels:\n  %.1f %.1f\n  %.1f %.1f\n", textureData[0], textureData[1], textureData[2], textureData[3]))
    ...
    for compareFunction in compareFunctions {
        ...
        print("compare: 0.5 \(compareFunction == .less ? "less than" : "greater than") texel (0,0 value: \(textureData[0]))")

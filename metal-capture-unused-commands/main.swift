...
//
// run with:
//    rm -rf trace.gputrace
//    METAL_CAPTURE_ENABLED=1 ./main
//    open trace.gputrace
//
import Metal
import MetalKit
...
func makePipeline(device: MTLDevice, vertexFunction: MTLFunction, fragmentFunction: MTLFunction, texture: MTLTexture) -> MTLRenderPipelineState {
...
func makeCommandBuffer(commandQueue: MTLCommandQueue, renderPipelineState: MTLRenderPipelineState, texture: MTLTexture) -> MTLCommandBuffer {
...
func main() {
    ...
    captureDescriptor.outputURL = URL(filePath: "trace.gputrace")
    do {
    ...
    let texture = makeTexture(device: device);
    let texture2 = makeTexture(device: device);
    ...
    let renderPipelineState = makePipeline(device: device, vertexFunction: vertexFunction, fragmentFunction: fragmentFunction, texture: texture)
    let renderPipelineState2 = makePipeline(device: device, vertexFunction: vertexFunction, fragmentFunction: fragmentFunction, texture: texture2)

    let commandBuffer = makeCommandBuffer(commandQueue: commandQueue, renderPipelineState: renderPipelineState, texture: texture)
    let _ = makeCommandBuffer(commandQueue: commandQueue, renderPipelineState: renderPipelineState2, texture: texture)
    ...
    let fileURL = URL(fileURLWithPath: "result2.png")
